...
if not IS_TTY:
    ...
    run = True
    _swaps = (0, 0, 1)
    swap_constr_, swap_sizing_, swap_equip_ = [bool(i) for i in _swaps]
...
def match_srf(act_srf, ref_areas, area_eps):
    ...
    def _lt_fn(diff):
        return diff <= area_eps

    def _diff_fn(ref_area):
        return abs(act_srf.grossArea() - ref_area)

    diffs = list(map(_diff_fn, ref_areas))
    diffs = list(filter(_lt_fn, diffs))
    assert len(diffs) > 0, len(diffs)
    return _assert_match(argmin(diffs))
