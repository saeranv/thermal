...
def main(osm_fpath, epw_fpath, mea_dpath):
    ...
    print(f"Saving osm to: {os.path.relpath(osm_fpath)}")
    osm_fpath_swap = dump_osm(osm_model, osm_fpath)
    print(f"Saving osw to: {os.path.relpath(osw_fpath_swap)}")
    _ = dump_osw(osw_dict, osw_fpath_swap)
